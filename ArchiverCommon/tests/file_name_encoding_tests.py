...
class FileNameEncodingTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.archive_formats = ['.zip', '.tar.gz']
        cls.archiver_tools = [bsdtar_tool, p7zip_tool]
        cls.utf_8_filename = 'พลัง'
        cls.create_utf8()

    @classmethod
    def get_tool_name(cls, archiver_tool):
        return pathlib.Path(archiver_tool.__name__).suffix
    ...
    def create_utf8(cls):
        ...
        for archive_format in cls.archive_formats:
            for archiver_tool in cls.archiver_tools:
                archive_file_path = os.path.join(cls.result_dirpath, f'{cls.get_tool_name(archiver_tool)}{archive_format}')
    ...
    def test_extract_utf8(self):
        for archive_format in self.archive_formats:
            for archiver_tool in self.archiver_tools:
                for archiver_tool_from in self.archiver_tools:

                    archive_file_path = os.path.join(self.result_dirpath, f'{self.get_tool_name(archiver_tool)}{archive_format}')
                    extracted_dir_path = os.path.join(self.result_extracted_dirpath, f"{os.path.basename(archive_file_path)}--{self.get_tool_name(archiver_tool_from)}")
